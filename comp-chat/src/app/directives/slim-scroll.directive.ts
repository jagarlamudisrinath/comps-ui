...
export class SlimScrollDirective implements OnInit, OnDestroy {
  ...
  public constructor(
    ...
    //this._options.height = this.setUpHeight();
  ...
  private hasParentClass(e: HTMLElement, className: string): any {
    if (!e) {
      return false;
    }
  ...
  private attachWheel(target: Window): void {
    ...
    target.addEventListener("mousewheel", this.onWheel, { passive: false });
  };
  ...
  private refresh(): void {
    ...
    if ("height" in this._options && this._options.height === "auto") {
      ...
      const height = this._me.parentElement && this._me.parentElement.clientHeight;
  ...
  private setUpHeight() {
    ...
    //let calSpace = viewPortHeight - this._me.offsetTop
    const actualHeight = viewPortHeight - footerHeight;
    const height = actualHeight * (+offset / 100);
    return height.toString();
  }

  private setup(): void {
    this._options.height = this.setUpHeight() + "px";
